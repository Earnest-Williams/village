...
def load_goods(paths):
    """
    Load goods definitions and return a map of per_unit_kg and discrete flag.

    Returns dict: { good_id: { 'per_unit_kg': float, 'discrete': bool } }
    """
    per_kg = {}
    for path_str in paths:
        ...
        if not pth.exists():
            continue
        data = yaml.safe_load(pth.read_text(encoding="utf-8")) or {}
...
def convert_amounts(mapping, per_kg_map, warnings):
    """
    Convert amounts from unit counts to mass (kg) using per_unit_kg mapping.
    Returns a dict of mass values.
    """
    out = {}
    for item, amt in (mapping or {}).items():
        if item not in per_kg_map:
            warnings.append(
                f"Unknown good '{item}' — using per_unit_kg=1.0 (fix or map manually)."
            )
            per = 1.0
        else:
...
def convert_recipe(old_id, old_obj, per_kg_map, warnings):
    """Convert a single legacy recipe object into v3.5 recipe dict (mass units)."""
    new = {}
    new["id"] = old_id
    new["facility"] = old_obj.get("facility")
    new["inputs"] = convert_amounts(old_obj.get("inputs", {}), per_kg_map, warnings)
    new["outputs"] = convert_amounts(
        old_obj.get("outputs", {}), per_kg_map, warnings
    )
    new["byproducts"] = convert_amounts(
        old_obj.get("byproducts", {}) or {}, per_kg_map, warnings
    )
    ...
    new["workers_required"] = int(
        old_obj.get("labor", old_obj.get("workers_required", 1))
    )
...
def main(args):
    """Main CLI: convert recipes and write recipes_converted.yaml"""
    oldp = Path(args.old)
    if not oldp.exists():
        print("Old recipes file not found:", oldp, file=sys.stderr)
        sys.exit(2)

    old = yaml.safe_load(oldp.read_text(encoding="utf-8")) or {}
    ...
    out = {"schema_version": "3.5", "file": Path(args.out).name, "recipes": new_recipes}
